...
static void check_owned(const USBDriver *usbp, const void *p, size_t size) {
  uintptr_t first = (uintptr_t)usbp;
  uintptr_t address = (uintptr_t)p;

  assert(address >= first);
  assert(address + size <= first + sizeof(*usbp));
}

static void check_driver(USBDriver *usbp) {
  const USBEndpointConfig *epcp = usbp->epc[0];

  check_owned(usbp, epcp, sizeof(*epcp));
  check_owned(usbp, epcp->in_state, sizeof(*epcp->in_state));
  check_owned(usbp, epcp->out_state, sizeof(*epcp->out_state));
  check_owned(usbp, epcp->setup_buf, 8U);

...
static void mfs_flash_acquire(MFSDriver *mfsp) {
  ...
  chDbgAssert(ferr == FLASH_NO_ERROR, "flash exclusive access failed");
}
...
static void mfs_flash_release(MFSDriver *mfsp) {
  ...
  chDbgAssert(ferr == FLASH_NO_ERROR, "flash exclusive release failed");
}
...
mfs_error_t mfsStart(MFSDriver *mfsp, const MFSConfig *config) {
  ...
  chDbgCheck((mfsp != NULL) && (config != NULL));
  chDbgAssert((mfsp->state == MFS_STOP) || (mfsp->state == MFS_READY) ||
...
mfs_error_t mfsStartTransaction(MFSDriver *mfsp, size_t size) {
  ...
  chDbgCheck((mfsp != NULL) && (size > ALIGNED_DHDR_SIZE));

...
#define ADC_CCR_DUAL_REGULAR            ADC_CCR_DUAL_FIELD(6U)
#define ADC_CCR_DUAL_INTERLEAVED        ADC_CCR_DUAL_FIELD(7U)
 ...
 * @note    ADC2's vector uses STM32_IRQ_ADC1_PRIORITY in this mode, as does
 *          the master DMA channel. Otherwise each DMA IRQ uses its ADC's
 *          STM32_IRQ_ADCx_PRIORITY.
 */
...
#error "ADC2 is owned by ADCD1 in dual mode"
#endif
...
#define adc_lld_driver_fields                                               \
  ...
  /* DMA request number.*/                                                  \
  uint8_t                           dreq;                                   \
  /* Conversion generation, guards callbacks that stop/restart.*/           \
  uint32_t                          sequence;                               \
  /* DMA buffers.*/                                                         \
  adc_dmabuf_t                      *dbuf;
#else
...
#define adc_lld_config_fields                                               \
  /* Common CCR settings, applied before enabling either ADC.               \
     DAMDF is driver-owned. Independent ADCs must use the same CCR.          \
     Changing this field requires stopping both drivers.*/                   \
  uint32_t                          ccr;                                    \

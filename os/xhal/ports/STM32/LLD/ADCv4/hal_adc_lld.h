 ...
 * @note    In dual mode, 32 selects packed pairs of up to 16-bit results.
 *          Each buffer word contains the master in the low half and the
 *          slave in the high half; num_channels counts both ADCs.
 */
 ...
 */
#if !defined(STM32_IRQ_ADC12_PRIORITY) || defined(__DOXYGEN__)
...
#if STM32_ADC_USE_ADC12 &&                                                  \
    !CH_IRQ_IS_VALID_PRIORITY(STM32_IRQ_ADC12_PRIORITY)
...
#if STM32_ADC_USE_ADC12 && (STM32_ADC12_CLOCK > STM32_ADCCLK_MAX)
#error "STM32_ADC12_CLOCK exceeding maximum frequency (STM32_ADCCLK_MAX)"
#endif

#if STM32_ADC_USE_ADC3 && (STM32_ADC3_CLOCK > STM32_ADCCLK_MAX)
...
#if STM32_ADC_USE_ADC12 || (STM32_ADC_USE_ADC3 && !STM32_ADC_ADC3_USE_BDMA)
...
#define adc_lld_driver_fields                                               \
  ...
  adc_lld_dma_reference_t    data;                                          \
  /* Identifies conversion restarts made by callbacks.*/                    \
  uint32_t                  sequence;                                      \
  /* DMA mode bit mask.*/                                                   \
  uint32_t                  dmamode;
#else
...
#define adc_lld_config_fields                                               \
  ...
  uint32_t                  difsel;                                         \
  /* Calibration mode, specify ADCALLIN and/or ADCALDIF bits in here.*/     \
  uint32_t                  calibration;

#if (STM32_ADC_DUAL_MODE == TRUE) || defined(__DOXYGEN__)
#define adc_lld_configuration_group_fields                                  \
  /* ADC CFGR register initialization data.                                 \
     NOTE: The DMNGT bits are enforced internally                           \
  ...
  uint32_t                  cfgr2;                                          \
  /* ADC CCR sensor enables only: VREFEN, TSEN and VBATEN.*/                \
  uint32_t                  ccr;                                            \

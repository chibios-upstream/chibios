...
/**
 * @file    SPIv4/hal_spi_lld.h
 * @brief   STM32 XHAL SPI subsystem low level driver header.
 *
 * @addtogroup SPI
 * @{
 */
...
/**
 * @brief   Circular and slave mode capabilities.
 */
...
/**
 * @brief   Chip select representation.
 */
...
#if !defined(STM32_SPI_DEFAULT_CFG1) || defined(__DOXYGEN__)
#define STM32_SPI_DEFAULT_CFG1             (SPI_CFG1_MBR_DIV128 | SPI_CFG1_DSIZE_8BITS)
#endif

#if !defined(STM32_SPI_DEFAULT_CFG2) || defined(__DOXYGEN__)
...
#define spi_lld_select_fields                                               \
  ioline_t ssline
#define SPI_DEFAULT_SELECT                                                  \
  .ssline = PAL_LINE(STM32_SPI_DEFAULT_PORT, STM32_SPI_DEFAULT_PAD)
#define spi_lld_select(spip)                                                \
  palClearLine(__spi_getfield(spip, ssline))
#define spi_lld_unselect(spip)                                              \
...
#define spi_lld_select_fields                                               \
  ...
  uint_fast8_t sspad
#define SPI_DEFAULT_SELECT                                                  \
  .ssport = STM32_SPI_DEFAULT_PORT, .sspad = STM32_SPI_DEFAULT_PAD
#define spi_lld_select(spip)                                                \
...
#else
#error "invalid STM32_SPI_SELECT_MODE"
#endif

#if (STM32_SPI_DEFAULT_CFG1 & SPI_CFG1_DSIZE_MASK) < SPI_CFG1_DSIZE_9BITS
#define SPI_DEFAULT_MODE                  SPI_MODE_FSIZE_8
#elif (STM32_SPI_DEFAULT_CFG1 & SPI_CFG1_DSIZE_MASK) < SPI_CFG1_DSIZE_17BITS
#define SPI_DEFAULT_MODE                  SPI_MODE_FSIZE_16
#else
#define SPI_DEFAULT_MODE                  SPI_MODE_FSIZE_32
#endif
...
{                                                                           \
  ...
  SPI_DEFAULT_SELECT,                                                       \
  .cfg1 = STM32_SPI_DEFAULT_CFG1,                                           \
  .cfg2 = STM32_SPI_DEFAULT_CFG2                                            \
}
...
#define spi_lld_driver_fields                                               \
  ...
  /* Whether the peripheral supports 4..32 bit frames.*/                    \
  bool                              full_feature;                           \
 ...
 * @note    SPI_MODE_FSIZE must match the storage width of CFG1.DSIZE:
 *          8 bits for 4..8, 16 bits for 9..16, or 32 bits for 17..32.
 *          Extra DMA settings must not override widths, increment modes,
 *          request selection or the transfer direction managed by the LLD.
 */
#define spi_lld_config_fields                                               \
  /* Chip select.*/                                                         \
  spi_lld_select_fields;                                                    \
